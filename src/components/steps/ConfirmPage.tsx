...
import {
    PaymentMeta, Service, OutletRootState, EnforcementType, ConsentCheckStatus, EnrichedService, Slot,
    PaymentPayload, ConfirmDateType, ConsentDraftEntry, ConsentDraftMap, CardType, CardData, ConsentModalPayload,
    SignatureType, AppointmentPayload, SubmitFinalConsentPayload, FormValues, FetchCustomerResponse
} from "@/types";
import { setUserDetails, clearUserDetails, setPayType } from "@/slices/appointmentSlice";
...
export default function ConfirmPage(): JSX.Element {
    ...
    const { tenantId, id: outletId, timeZone, image, outletName, address } = useSelector((state: OutletRootState) => state.booking.outletDetails);
    const { staff, selectedServices, selectedProfessional } = useSelector((state: RootState) => state.booking.service);
    const { selectedSlotIds, selectedDate, selectedTime, selectedSlotIndexes, slots } = useSelector((state: RootState) => state.booking.slots);
    const { userDetails, bookingMode, tipPct, payType } = useSelector((state: RootState) => state.booking.appointment);
    ...
    const allSlots: Slot[] = [...(slots?.morning ?? []), ...(slots?.afternoon ?? []), ...(slots?.evening ?? [])];
    ...
    const onFormSubmit: SubmitHandler<FormValues> = async (data): Promise<void> => {
        const payload = {
            ...data,
            phone: data?.phone ?? "",
        };
                            ...
                            {payType &&
                                <form className="aaravpos-tp-10" onSubmit={handleSubmit(onFormSubmit)}>
                                    ...
                                    <div className="aaravpos-center-items">
                                        ...
                                        >
                                            <span className="aaravpos-btn-content">Confirm Details</span>
                                        </button>
                                    </div>
                                </form>
                            }
...
function Card({ children }: CardProps): JSX.Element {
    return (
        <div className="arravpos-card-box">{children}</div>
    );
}
...
function Avatar({ pro }: any): JSX.Element {
    return (
        <>
            {pro.imageUrl ? (
                <img src={pro.imageUrl} alt={pro.name} className="arravpos-profile-image" />
            ) : (
                <div className="arravpos-profile-avatar" style={{ background: pro.color ?? "#111" }}>
